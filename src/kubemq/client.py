...
def classify_error(error_msg: str) -> ErrorType:
    ...
    if any(term in error_msg for term in ["connection", "connect", "network", "unreachable"]):
        return ErrorType.CONNECTION
    
    # Temporary/transient errors
    if any(term in error_msg for term in ["timeout", "unavailable", "temporary", "overload"]):
...
class KubeMQClient(Connection):
    ...
    def _connect(self):
        ...
        try:
            ...
            self.logger.error(f"Error connecting to KubeMQ server: {error_msg} (Type: {error_type.value})")
            self.last_error = e
            self.metrics.track_error(ErrorCategory.CONNECTION, f"Error connecting to KubeMQ server: {error_msg}")
    ...
    def _disconnect(self):
        ...
        try:
            ...
            self.last_error = e
            self.metrics.track_error(ErrorCategory.CONNECTION, f"Error disconnecting from KubeMQ server: {error_msg}")
            raise KubeMQConnectionError(
                f"Error disconnecting from KubeMQ server: {error_msg} (Type: {error_type.value})"
            )
        finally:
                ...
                try:
                    ...
                    for message in poll_response.messages:
                        try:
                            ...
                            self.metrics.track_message_received(poll_duration_ms)
        ...
        async def _send_message():
            ...
            try:
                ...
                if result.is_error:
                    ...
                    error_type = classify_error(error_msg)
                    self.logger.error(f"Error sending message: {error_msg} (Type: {error_type.value})")
                    self.last_error = Exception(error_msg)
                    self.metrics.track_error(
                        ErrorCategory.SEND, 
    ...
    async def check_health(self) -> Dict[str, Any]:
        ...
        if self.last_error is not None:
            ...
            error_type = classify_error(error_msg)
            health_result["errors"]["last_error"] = f"{error_msg} (Type: {error_type.value})"
        ...
        if self.is_connected:
            try:
                ...
                health_result["details"]["latency_msec"] = round(ping_duration_ms * 100) / 100
            except Exception as e:
                error_msg = str(e)
                error_type = classify_error(error_msg)
                ...
                health_result["errors"]["connectivity_check"] = f"{error_msg} (Type: {error_type.value})"
